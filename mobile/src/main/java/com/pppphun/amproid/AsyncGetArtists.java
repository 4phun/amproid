...
public class AsyncGetArtists extends AsyncTask<Void, Void, Vector<HashMap<String, String>>>
{
    ...
    protected void onPostExecute(Vector<HashMap<String, String>> artists)
    {
        ...
        if (defective > 0) {
            ...
            results.add(new MediaBrowserCompat.MediaItem(new MediaDescriptionCompat.Builder().setMediaId(Amproid.getAppContext().getString(R.string.item_root_id)).setTitle(String.format(Amproid.getAppContext().getString(R.string.error_defective_artists), defective)).build(), MediaBrowserCompat.MediaItem.FLAG_BROWSABLE));
        }
